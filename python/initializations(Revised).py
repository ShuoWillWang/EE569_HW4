...
#insert the K-means initialization file
def KM1():
    matrix = numpy.loadtxt('D:/EE569_Assignment/4/C++/Kmeans/x64/Debug/Weight_W.txt');
    temp = numpy.resize(matrix, (1, 5 * 5 * 3 * 6))
    temp = tf.convert_to_tensor(temp, tf.float32)
    initial = tflearn.layers.core.reshape(temp, [5, 5, 3, 6])
    return initial
...
def KM3():
    matrix = numpy.loadtxt('D:/EE569_Assignment/4/C++/Kmeans3/x64/Debug/Weight_W.txt');
    temp = numpy.resize(matrix, (1, 5 * 5 * 16 * 120))
    temp = tf.convert_to_tensor(temp, tf.float32)
    initial = tflearn.layers.core.reshape(temp, [5 * 5 * 16, 120])
    return initial
